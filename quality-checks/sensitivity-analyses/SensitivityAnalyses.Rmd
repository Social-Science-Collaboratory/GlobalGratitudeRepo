...
en_tau_p <- sapply(en.m.ic, function(model) (model$sigma2[1]))
```
...
qc_tau_p <- sapply(qc.m.ic, function(model) (model$sigma2[1]))
```
...
all_tau_p <- sapply(m.ic, function(model) (model$sigma2[1]))
```
...
en_tau <- data.frame(
  ...
  en_p = en_tau_p,
  all_p = all_tau_p,
  p_diff = all_tau_p - en_tau_p,
  p_ratio = en_tau_p / all_tau_p,
...
qc_tau <- data.frame(
  ...
  qc_p = qc_tau_p,
  all_p = all_tau_p,
  p_diff = all_tau_p - qc_tau_p,
  p_ratio = qc_tau_p / all_tau_p,
...
results_df <- results_df %>%
  left_join(en_results_df, by = c("outcome")) %>%
