...
# Gratitude
gra <- psych::alpha(DF %>% select(grateful, appreciative, thankful))$total$raw_alpha

# Positive affect
pas <- psych::alpha(DF %>% select(happy, satisfied, content, joyful, pleased))$total$raw_alpha

# Optimism
opt <- psych::alpha(DF %>% select(optimistic, hopeful))$total$raw_alpha

# Negative affect
neg <- psych::alpha(DF %>% select(sad, depressed, anxious, nervous))$total$raw_alpha
...
# Sense of self
sen <- psych::alpha(DF %>% select(self_image_circle, self_image))$total$raw_alpha

# Combine into a tidy table
overall_alpha <- data.frame(
  outcome = c("Gratitude", "Positive Affect", "Optimism", "Negative Affect",
              "Indebtedness", "Envy", "Life Satisfaction", "Sense of Self"),
  alpha = c(gra, pas, opt, neg, ind, env, lif, sen)
)

write.csv(overall_alpha,
  file = here(
    "data/output",
