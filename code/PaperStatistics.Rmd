...
ef <- read.csv(file = here("quality-checks/sensitivity-analyses", "si_table_4.csv")) %>% filter(Sample == "Full") %>% select (Outcome,Cohen.s.d)

print(ef)

# Overall tau
tau <- read.csv(file = here("quality-checks/sensitivity-analyses", "si_table_4.csv")) %>% filter(Sample == "Full") %>% select (Outcome, τc,τp)
