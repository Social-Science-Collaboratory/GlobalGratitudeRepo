...
pred_df <- do.call(rbind, lapply(seq_along(m.ic), function(i) {
  ...
  # 4) Extract tau^2 components
    tau2_practice <- mod$sigma2[1]
    var_country  <- mod$sigma2[2]
  
  z <- qnorm(0.95)
  
  # Heterogeneity-only intervals
  lwr_practice_heter <- pred - z * sqrt(tau2_practice + (0.0448^2))
  upr_practice_heter <- pred + z * sqrt(tau2_practice + (0.0448^2))
  
  lwr_country_heter <- pred - z * sqrt(var_country)
  upr_country_heter <- pred + z * sqrt(var_country)
  
  lwr_both_heter <- pred - z * sqrt(tau2_practice + var_country)
  upr_both_heter <- pred + z * sqrt(tau2_practice + var_country)
  
  # Full prediction intervals for a single new study
  var_practice_full <- var_beta + tau2_practice + v_new
  var_country_full  <- var_beta + var_country  + v_new
  var_both_full     <- var_beta + tau2_practice + var_country + v_new
