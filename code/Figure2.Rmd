...
pred_df <- do.call(rbind, lapply(1:length(m.ic), function(i) {
  mod <- m.ic[[i]]
  ...
  sd_practice <- sqrt(mod$sigma2[1])
  sd_country  <- sqrt(mod$sigma2[2])
  
  # Prediction intervals
  lwr_practice <- pred - 1.96 * sd_practice
  upr_practice <- pred + 1.96 * sd_practice
  
  lwr_country <- pred - 1.96 * sd_country
  upr_country <- pred + 1.96 * sd_country
  
  # Combine into a data frame
  data.frame(
    effect_size = effect_size_columns[i],
    pred = pred,
    lwr_practice = lwr_practice,
    upr_practice = upr_practice,
    lwr_country = lwr_country,
    upr_country = upr_country
    )
}))
