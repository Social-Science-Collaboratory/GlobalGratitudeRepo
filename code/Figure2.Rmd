...
# open data
df <-
  read.csv(file = here("data/output", "unique_country_effect_sizes.csv")) %>%
...
d.i <- lapply(effect_size_columns, function(effect_size) {
  d.i <- df %>%
    ...
    # add [jittered] y-value
    mutate(y = -0.02, y = jitter(y, .1)) %>%

    # labels most extreme observations
    mutate(
      label = if_else(m == min(m), true = intervention_condition, false = ""),
      label = if_else(m == max(m), true = intervention_condition, false = label)
    ) %>%
...
m.d <- lapply(seq_along(effect_size_columns), function(i) {
  ...
  plot <- ggplot() +
    ...
    geom_segment(
      aes(x = model$ci.lb, xend = model$ci.ub, y = 5.7, yend = 5.7),
      color = "grey30"
    ) +
    ...
    stat_function(
      fun = dnorm,
      args = list(mean = model$b[1], sd = sqrt(model$sigma2[1])),
      color = "#E64B35"
    ) +
    ...
    stat_function(
      fun = dnorm,
      args = list(mean = model$b[1], sd = sqrt(model$sigma2[2])),
      color = "#3C5488"
    ) +
    # Add tau_ci label
    geom_text(
      aes(x = 0.7, y = 11.5, label = label_text),
      color = "black",
      hjust = 1,
      vjust = 1,
      size = 4
  ...
  if (i == 1) {
    plot <- plot +
      ...
      geom_segment(
        aes(x = 0.28, xend = 0.52, y = 10.2, yend = 10.2),
        color = "black",
        size = 0.5
      )
  }
...
practice_pred_df <- do.call(
  rbind,
  lapply(seq_along(m.i), function(i) {
    mod <- m.i[[i]]
...
# Save data frame
write.csv(pred_df, file = here('data/output', "prediction_intervals.csv"))
```
