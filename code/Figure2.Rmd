...
```{r}
# Build a data frame with fixed effect, practice-level and country-level prediction intervals
pred_df <- do.call(rbind, lapply(seq_along(m.ic), function(i) {
  mod <- m.ic[[i]]
  
  # 1) Predicted mean (population-level)
  pred <- predict(mod, digits = 3)$pred
  
  # 2) Fixed-effect variance
  Vbeta <- as.matrix(vcov(mod))
  var_beta <- if (ncol(Vbeta) >= 1) Vbeta[1,1] else 0
  
  # 3) Sampling variance to represent a plausible new study
  v_new <- if (!is.null(mod$vi)) median(mod$vi, na.rm = TRUE) else 0
  
  # 4) Extract tau^2 components
  tau2_all <- as.numeric(mod$sigma2)
  if (length(tau2_all) == 0) {
    var_practice <- var_country <- 0
  } else if (length(tau2_all) == 1) {
    var_practice <- tau2_all[1]
    var_country  <- 0
  } else {
    var_practice <- tau2_all[1]
    var_country  <- tau2_all[2]
  }
  
  z <- qnorm(0.975)
  
  # Heterogeneity-only intervals
  lwr_practice_heter <- pred - z * sqrt(var_practice)
  upr_practice_heter <- pred + z * sqrt(var_practice)
  
  lwr_country_heter <- pred - z * sqrt(var_country)
  upr_country_heter <- pred + z * sqrt(var_country)
  
  lwr_both_heter <- pred - z * sqrt(var_practice + var_country)
  upr_both_heter <- pred + z * sqrt(var_practice + var_country)
  
  # Full prediction intervals for a single new study
  var_practice_full <- var_beta + var_practice + v_new
  var_country_full  <- var_beta + var_country  + v_new
  var_both_full     <- var_beta + var_practice + var_country + v_new
  
  lwr_practice_full <- pred - z * sqrt(var_practice_full)
  upr_practice_full <- pred + z * sqrt(var_practice_full)
  
  lwr_country_full <- pred - z * sqrt(var_country_full)
  upr_country_full <- pred + z * sqrt(var_country_full)
  
  lwr_both_full <- pred - z * sqrt(var_both_full)
  upr_both_full <- pred + z * sqrt(var_both_full)
  ...
  data.frame(
    ...
    # heterogeneity-only
    lwr_practice_heter = lwr_practice_heter,
    upr_practice_heter = upr_practice_heter,
    lwr_country_heter  = lwr_country_heter,
    upr_country_heter  = upr_country_heter,
    lwr_both_heter     = lwr_both_heter,
    upr_both_heter     = upr_both_heter,
    
    # full PIs
    lwr_practice_full = lwr_practice_full,
    upr_practice_full = upr_practice_full,
    lwr_country_full  = lwr_country_full,
    upr_country_full  = upr_country_full,
    lwr_both_full     = lwr_both_full,
    upr_both_full     = upr_both_full,
